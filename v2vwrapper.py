...
negative_prompt='nipple nudity boobs boob breast naked bosom bad proportion grainy blurry cartoon'
...
class StreamV2VWrapper:
    def __init__(
        ...
        self.device = device
        if self.device == "cuda":
            self.dtype = dtype  # allow float16
        else:
    ...
    def inject(self,
        ...
        self.stream.prompt_embeds = encoder_output[0].repeat(self.stream.batch_size, 1, 1)
        self.stream.null_prompt_embeds = encoder_output[1]

        if self.stream.use_denoising_batch and self.stream.cfg_type == "full":
            uncond_prompt_embeds = encoder_output[1].repeat(self.stream.batch_size, 1, 1)
        elif self.stream.cfg_type == "initialize":
            uncond_prompt_embeds = encoder_output[1].repeat(self.stream.frame_bff_size, 1, 1)

        if self.stream.guidance_scale > 1.0 and (
            self.stream.cfg_type == "initialize" or self.stream.cfg_type == "full"
        ):
    ...
    def img2img(
        ...
        if isinstance(image, str) or isinstance(image, Image.Image):
            # print(f"--sPreprocessing image img2img v2vwrpper")
            image = self.preprocess_image(image.to('cpu')).to(self.device, self.dtype)

        image_tensor = self.stream(image)
        # print(f"--Postprocessing image img2img v2vwrpper")
    ...
    def _load_model(
        ...
        3. Loads the VAE model from the vae_id if needed. NVMD VAE LOADED AUTOMATICALLY
...
".
        ...
        pipeline_cls = StableDiffusionXLPipeline if self.sd_xl else self.pipeline_cls

        try:
            # Attempt to load the model from a local directory
            # print(self.pipe_kwargs)
            pipe = pipeline_cls.from_pretrained(model_id_or_path).to(device=self.device, dtype=self.dtype,**self.pipe_kwargs)
            ...
            try:
                ...
                logging.error(f"Failed to load model {model_id_or_path} from Hugging Face :\n{e0}\n{e} ...(do you need to run huggingface-cli login for BS SD rules? )")
        ...
        if not self.sd_turbo:
            ...
            #     stream.pipe.load_lora_weights(turbo_lora)
            #     stream.pipe.fuse_lora()
        ...
        assert vae_id is not None, "vae_id must be specified if use_tiny_vae is False"
        # print('vae kwargs:', vae_kwargs)
        # print(vae_cls)
        # print(stream.vae,'already loaded?')
        if not hasattr(stream, 'vae') or stream.vae is None:
            # Load the VAE model
            stream.vae = vae_cls.from_pretrained(vae_id).to(
                    device=pipe.device, dtype=pipe.dtype,**vae_kwargs
                )
        ...
        try:
            ...
            if acceleration == "tensorrt":
                ...
                if not os.path.exists(vae_encoder_path):
                    ...
                    vae_encoder = TorchVAEEncoder(stream.vae).to(stream.device)
